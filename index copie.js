...
app.get('/dossier', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role === 'client') {
      // Client récupère son propre dossier uniquement
      ...
      if (targetUserId) {
        // Dossier complet client ciblé
        const dossierDoc = await db.collection('users').doc(targetUserId)
                                  .collection('dossier_client').doc(targetUserId).get();
...
app.post('/RouteEnregistrementTraing', authenticateToken, async (req, res) => {
  ...
  try {
    // Récupère l'email du client dans le corps de la requête (et non celui du coach)
    const clientEmail = (req.body.email || '').toLowerCase();

    if (!clientEmail) {
      return res.status(400).json({ error: "Email client manquant dans la requête." });
    }

    const { entrainements } = req.body;

    // Validation des entrainements
    if (!Array.isArray(entrainements) || entrainements.length === 0) {
      return res.status(400).json({ error: 'Entraînement vide.' });
    }

    const sanitizedEmail = clientEmail.replace(/[@.]/g, '_');
      ...
      if (typeTraining === 'cross-training') {
        ...
        exercices.forEach((exo) => {
          ...
          nouvellesPerformances.push({
            ...
            nom: exo.nom,
            series: exo.series ?? 0,
            reps: exo.repetitions ?? 0,   // Défaut à 0 si undefined
            charges: [
              {
